...
import { createQuote, getQuotes } from '../api/queries/quotes'
  ...
  return (
    <div>
      <PageHeader
        ...
        onSecondaryAction={() => window.location.assign('/clients')}
      />
      ...
      <Card>
        ...
        {isLoading ? (
          ...
          <div className="p-4"><StateBlock title="No se pudieron cargar los presupuestos" description="Comprueba que blume-api esté arrancada en el puerto 4000." /></div>
                  ...
                  <tr key={quote.id}>
                    ...
                    <td className="px-4 py-4 sm:px-5">
                      <div className="flex justify-end gap-2">
                        {[{ label: 'Ver presupuesto', icon: Eye }, { label: 'Enviar presupuesto', icon: Send }, { label: 'Descargar presupuesto', icon: Download }].map((action) => (
                          <button key={action.label} type="button" className="grid size-9 place-items-center rounded-md border border-slate-200 text-slate-600 transition hover:bg-slate-50" aria-label={action.label} title={action.label}>
      ...
      <Modal title="Crear presupuesto" open={isModalOpen} onClose={() => setIsModalOpen(false)}>
        ...
        >
          ...
          <label>
            ...
            <select className="mt-2 h-10 w-full rounded-md border border-slate-200 px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" name="clientId" required>
          ...
          <label>
            ...
            <select className="mt-2 h-10 w-full rounded-md border border-slate-200 px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" name="catalogItemId" required onChange={(event) => setSelectedCatalogItemId(event.target.value)}>
          ...
          {createMutation.isError ? <p className="sm:col-span-2 text-sm text-rose-700">No se pudo crear el presupuesto. Revisa que el código no exista ya.</p> : null}
